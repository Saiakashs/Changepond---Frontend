function checkData(){
    ...
    let regPass= "/^[a-zA-Z0-9!@#\$%\^\&*_=+-]{8,18}$/g";
    ...
    if(!upass.trim().match(regPass)){
        window.alert("Passsword should contain mininum 8 chracters and maximum 18 characters, atleast one uppercase letter, one lowercase letter,one number,one special symbol");
        return false;
    }
    ...
    if(ugender[0].checked==false && ugender[1].checked==false && ugender[2].checked==false && ugender[3].checked==false){
